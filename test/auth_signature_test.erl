...
valid_signature() ->
  ...
  ?assertEqual(ok, auth_signature:validate(<<"SocketId:private-channel">>, Auth)),
...
invalid_key() ->
  ...
  ?assertEqual(error, auth_signature:validate(<<"SocketId:private-channel">>, <<"Auth">>)),
  ?assert(meck:validate(authentication)).

invalid_signature() ->
  meck:expect(application, get_env, 2, {ok, <<"secret">>}),
  meck:expect(authentication, check_key, 1, ok),
  ?assertEqual(error, auth_signature:validate(<<"private-channel">>, <<"Wrong:Auth">>)),
  ?assert(meck:validate(application)),
  ?assert(meck:validate(authentication)).
